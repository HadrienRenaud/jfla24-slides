...
\title{Executable semantics of ASL}
...
\begin{frame}[fragile]{Objectifs}
  ...
  \begin{tikzpicture}[remember picture, outer sep=4.8pt, >=Stealth]
    \node (instr) at (0, 3) [anchor=south, align=center]
      {\texttt{CAS Xs, Xt, [Xn]}};
    \node (code) at (-1,0) [anchor=east, text width=.4\textwidth, inner ysep=-1em]
      {\lstinputlisting%
        %[frame=shadowbox]%
...
\begin{frame}{Compare-And-Swap \texttt{CAS Xs, Xt, [Xn]}}
  ...
  \begin{figure}
    \includegraphics%
      [width=\textwidth, height=.9\textheight, keepaspectratio]%
      {graphs/old/cas-ok.pdf}
    \caption{Graphe g\'en\'er\'e par \texttt{herd7} pour l'instruction \texttt{CAS Xs, Xt, [Xn]} dans le cas ok}
  \end{figure}
\end{frame}
...
\begin{frame}[fragile]{Objectifs}
  ...
  \begin{tikzpicture}[remember picture, outer sep=4.8pt]
    \node (code) at (-1,0) [anchor=east, text width=.4\textwidth]
      {\lstinputlisting%
        %[basicstyle=\ttfamily\tiny]%
        {asl/cas.asl}};
    \node (graph) at (1,0) [anchor=west, text width=.4\textwidth]
...
\begin{frame}{D\'ependances intr-instruction de contr\^ole}
